...
class IOConnection implements IOCallback {
  ...
  private synchronized void connectTransport() {
    ...
    if (protocols.contains(WebsocketTransport.TRANSPORT_NAME)) {
      ...
      error(new SocketIOException("Server supports no available transports. "
          + "You should reconfigure the server to support a available transport"));
      return;
    }
    transport.connect();
  }
   ...
   * @return an {@link IOAcknowledge} instance, may be <code>null</code>
   */
      ...
      public void ack(JsonElement... args) {
        ...
        for (JsonElement o : args) {
          try {
            ...
            error(new SocketIOException("You can only put values in IOAcknowledge."
                + "ack() which can be handled by JSONArray.put()", e));
          }
        }
   ...
   *
   * @param ee an exception
   */
  private void error(SocketIOException ee) {
    for (SocketIO socket : sockets.values()) {
      socket.getCallback().onError(ee);
    }
    cleanup();
  }
  ...
  public void emit(SocketIO socket, String event, IOAcknowledge ack, Object... args) {
    try {
      ...
      IOMessage message = new IOMessage(IOMessage.TYPE_EVENT,
          socket.getNamespace(), jobj.toString());
      synthesizeAck(message, ack);
      sendPlain(message.toString());

    } catch (JsonParseException e) {
      error(new SocketIOException("Error while emitting an event."
          + " Make sure you only try to send arguments, which can be serialized into JSON."));
    }
  ...
  private class HearbeatTimeoutTask extends TimerTask {
    ...
    @Override
    public void run() {
      error(new SocketIOException("Timeout Error. "
          + "No heartbeat from server within life time of the socket. closing.", lastException));
    }
  }
